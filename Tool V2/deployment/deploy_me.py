#!/usr/bin/env python3
"""
Deploy Tool V2 Streamlit app to Posit Connect using ``rsconnect-python``.
...
def _load_dotenv() -> None:
    """Load repo root and Tool V2 ``.env`` if ``python-dotenv`` is installed."""
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(
        description="Deploy Tool V2 Streamlit app to Posit Connect (rsconnect-python)."
    )
    p.add_argument(
        "--server",
        default=None,
    ...
    p.add_argument(
        ...
        type=Path,
        default=root,
        help="Directory to bundle (default: Tool V2 root).",
    )
    p.add_argument(
        "--entrypoint",
        default="app.py",
    ...
    if not app_id and title is None:
        ...
        title = f"Measles Risk Tool V2 [{suffix}]"

...
By default fetches only vaccine = '≥1 Dose MMR'. Uses SOCRATA_APP_TOKEN from .env.
Fetched data is cleaned every run to MMR vaccine rows only.
...
Usage:
  ...
  python call_cdc_child_vax.py --unique vaccine
"""
...
PROJECT_ROOT = _SCRIPT_DIR.parent  # project root for .env
try:
...
DEFAULT_WHERE = 'vaccine = "≥1 Dose MMR"'
MMR_VACCINE_VALUES = {"≥1 Dose MMR"}
...
def clean_child_vax_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "vaccine" not in df.columns:
        print(f"Warning: 'vaccine' column not found; skipping clean. Columns: {list(df.columns)}", file=sys.stderr)
        return df
...
def main():
    ...
    parser.add_argument("--where", type=str, default=None, help="SoQL WHERE clause (default: vaccine = '≥1 Dose MMR'); use '' for no filter")
    ...
    parser.add_argument("--unique", type=str, metavar="COLUMN", help="Print distinct values for COLUMN (e.g. vaccine); use --schema to see columns")
